...
## The Plan
My plan for software engineering and design is to improve the software. The software will be improved by creating more readable output, more user-friendly input, and a well-rounded 'update' module for the API which will help to deliver better value. The API update module needs to better reflect the industry specific goals for update functionality by allowing the user to update any field in a specified document, not just the “volumes” field. I will also remove unused code, improve commenting and try to fix any bugs I come across.

The planned objectives for algorithms and data structures is to add to the complexity of the artifact by creating modules whose algorithms will allow for field deletion (for both the internal and API service), internal document retrieval and menu execution. Currently there is no menu that makes service selection easy, and no 'read' module for the internal service.

My plan for database enhancement is to apply more advanced MongoDB concepts through investigation and performance tuning on the database. I hope to meet the goal of improving database efficiency by improving its reads and writes through administrative methods.
